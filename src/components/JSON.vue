...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <ul>
        <li v-for="author in authors" :key="author.id">
          <span v-if="author.name == 'George Orwell'" :class="state">{{ author.name }} ({{ author.birthYear }})</span>
          <span v-if="author.name != 'George Orwell'">{{ author.name }} ({{ author.birthYear }})</span>
        </li>
      </ul>
      ...
      <ul>
        <li v-for="author in austen" :key="author.id">
          <div v-for="work in author.famousWorks" :key="work.title">
            year: {{ work.year }} - title: {{ work.title }}
          </div>
        </li>
      </ul>
    </section>
    ...
    <section class="lab-section">
      ...
      <ul>
          <!-- <li v-for="(key,value) in bookstores" :key="(key,value).name">
            {{ value }}:{{ key }}  //实际上正确顺序是`(value, key, index)`
          </li> -->
          <!-- <li v-for="item in bookstores" :key="item.name">
              {{ item.name }}
          </li> -->
          <li>
          {{ bookstores.name }}
      ...
      <ul>
        <li v-for="(days, dayType) in bookstores.openingHours" :key="days">
          <h2>{{ dayType }}</h2>
          <div v-for="(value,key) in days" :key="key">
            {{ key }}: {{ value }}
          </div>
        </li>
      </ul>
      ...
      <p>We operate in:</p>
      <ul>
        <li v-for="item in bookstores.topSellers" :key="item">
          {{ item }}
        </li>
      </ul>
      <p>Our #1 seller:</p>
    ...
    <section class="lab-section">
      ...
      <p class="message success" v-show="showMessage">✨ You're a Vue superstar! ✨</p>
    ...
    <section class="lab-section">
      ...
      <span class="highlight">
        1.1 (static Attribute Binding) George Orwell
      </span>
      <br>
      <span :class="state">
        2.1 (Class Binding) George Orwell
      </span>
      <br>
      <button @click="isHighLight = !isHighLight">SwitchHighLight</button>
      <span :class="{ highlight: isHighLight }">
        2.2 (Class Binding) George Orwell
      </span>
      <br>
      <span :style="{ color: textColor, fontSize: fontSize + 'px' }">
        3. (Style Binding) George Orwell
      </span>
    </section>
  </div>
</template>
...
const orwell = computed(() => {
  ...
  return authors.flatMap((author) => author.name=="George Orwell")
});
